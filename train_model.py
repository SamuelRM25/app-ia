...
<body>
    ...
    <script>
        ...
        trainButton.addEventListener('click', () => {
            ...
            // Enviar solicitud con manejo mejorado de errores
            fetch(`${API_URL}/train`, {
                method: 'POST',
                body: formData,
                // Agregar estas opciones para mejorar el manejo de CORS
                mode: 'cors',
            ...
            .then(response => {
                if (!response.ok) {
                    if (response.status === 404) {
                        throw new Error('Endpoint de entrenamiento no encontrado. Verifique que el backend tenga la ruta /train implementada.');
                    }
            ...
            .catch(error => {
                ...
                trainResultContent.innerHTML = `<p>Error: ${error.message}</p>
                    <p>Asegúrese de que el servidor esté funcionando correctamente y que tenga implementado el endpoint /train.</p>`;
            });
        });
    </script>
</body>
